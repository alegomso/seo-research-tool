...
export class PromptTemplateEngine {
  ...
  static renderPrompt(templateId: string, variables: { [key: string]: any }): string {
  ...
  static validateTemplateVariables(templateId: string, providedVariables: { [key: string]: any }): {
    isValid: boolean;
  ...
  static listTemplates(): Array<{ id: string; name: string; description: string; category: string }> {
  ...
  static createCustomPrompt(
    ...
    if (context) {
      ...
      if (context.goals) prompt += `- Business Goals: ${context.goals.join(', ')}\n`;
      if (context.tone) prompt += `- Preferred Tone: ${context.tone}\n`;
    }
