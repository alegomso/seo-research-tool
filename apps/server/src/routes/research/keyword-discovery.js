...
async function keywordDiscoveryRoutes(fastify) {
    // Start keyword discovery process
    fastify.post('/keyword-discovery', async (request, reply) => {
        try {
            const userId = request.user?.id;
            if (!userId) {
                return reply.status(401).send({ error: 'Authentication required' });
            }
            const { seedKeywords, location, language, includeQuestions, includeLongTail, minSearchVolume, maxKeywordDifficulty, analysisDepth, projectId } = request.body;
            // Validate input
            ...
            const query = await prisma.query.create({
                data: {
                    userId,
                    projectId,
                    type: client_1.QueryType.KEYWORD_DISCOVERY,
                    parameters: {
                        seedKeywords,
                        location,
                        language,
    ...
    fastify.get('/keyword-discovery/:queryId', async (request, reply) => {
        try {
            const userId = request.user?.id;
            if (!userId) {
                return reply.status(401).send({ error: 'Authentication required' });
    ...
    fastify.get('/keyword-discovery/:queryId/status', async (request, reply) => {
        try {
            const userId = request.user?.id;
            if (!userId) {
                return reply.status(401).send({ error: 'Authentication required' });
...
exports.default = keywordDiscoveryRoutes;
// Process keyword discovery workflow
async function processKeywordDiscovery(queryId) {
    try {
        ...
        await updateTaskStatus(keywordIdeasTask.id, client_1.TaskStatus.IN_PROGRESS, { dataForSEOTaskId: keywordTaskId });
        ...
        await updateTaskStatus(volumeTask.id, client_1.TaskStatus.IN_PROGRESS, { dataForSEOTaskId: volumeTaskId });
        ...
        const processedKeywords = await processKeywordResults(keywordIdeasResult, volumeResult, params);
        ...
        await updateTaskStatus(keywordIdeasTask.id, client_1.TaskStatus.COMPLETED, { datasetId: dataset.id });
        await updateTaskStatus(volumeTask.id, client_1.TaskStatus.COMPLETED);
...
async function processKeywordResults(keywordIdeasResult, volumeResult, params) {
    const keywords = [];
    ...
    if (keywordIdeasResult.tasks?.[0]?.result) {
        ...
        for (const item of ideas) {
            if (item.keywords) {
                keywords.push(...item.keywords.map((kw) => ({
                    keyword: kw.keyword,
    ...
    const filteredKeywords = keywords.filter(keyword => {
        // Apply volume filter
        if (keyword.searchVolume < params.minSearchVolume)
            return false;
...
function detectIntent(keyword) {
    ...
    if (commercial.some(word => lowerKeyword.includes(word)))
        return 'commercial';
    if (transactional.some(word => lowerKeyword.includes(word)))
        return 'transactional';
    if (navigational.some(word => lowerKeyword.includes(word)))
        return 'navigational';
...
function calculateOpportunityScore(keyword, params) {
    let score = 0;
    ...
    if (keyword.searchVolume >= 10000)
        ...
        score += 20;
    else if (keyword.searchVolume >= 100)
        score += 10;
    // Competition score (0-30 points) - lower is better
    if (keyword.competitionLevel === 'LOW')
        score += 30;
    else if (keyword.competitionLevel === 'MEDIUM')
        score += 15;
    else if (keyword.competitionLevel === 'HIGH')
        score += 5;
    ...
    if (keyword.intent === 'transactional')
        ...
        score += 15;
    else if (keyword.intent === 'informational')
        score += 10;
    // Trend score (0-10 points)
    if (keyword.trend === 'up')
        score += 10;
    else if (keyword.trend === 'stable')
        score += 5;
